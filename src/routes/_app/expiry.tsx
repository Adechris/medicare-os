...
function ExpiryPage() {
  ...
  const valueExpiring = list
    .filter((m) => m.days <= 30)
    .reduce((s, m) => s + m.stock * m.buyingPrice, 0);

  return (
    <div>
      <PageHeader
        title="Expiry Alerts"
        description="Stay ahead of stock that's about to expire"
        actions={
      ...
      <Card className="p-4 mt-4">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="ml-auto text-xs text-muted-foreground">
            Total value at risk:{" "}
            <span className="font-semibold text-foreground">{formatNaira(valueExpiring)}</span>
          </div>
        </div>
                      ...
                      <td className="py-2.5 pr-3">
                        <span
                          className={cn(
                            ...
                            critical
                              ? "bg-red-100 text-red-700 dark:bg-red-500/15 dark:text-red-300 animate-pulse-danger"
                              : warn
...
function SummaryCard({
  ...
  count: number;
  label: string;
  tone: "critical" | "urgent" | "warning" | "ok";
  ...
  return (
    <Card className="p-4">
      <div className="flex items-center gap-3">
        <div
          className={cn("h-10 w-10 rounded-lg flex items-center justify-center", map.bg, map.text)}
        >
