...
function ExpensesPage() {
  ...
  (exps.data||[]).forEach(e=>{ byCat[e.category]=(byCat[e.category]||0)+e.amount; });

  return (
    <div>
      <PageHeader title="Expenses" description="Track operating costs and budgets"
        actions={<Button onClick={()=>setAddOpen(true)}><Plus className="h-4 w-4"/> Add Expense</Button>}/>

      <div className="grid gap-3 grid-cols-2 lg:grid-cols-4 mb-4">
        <Card className="p-4"><div className="text-[11px] uppercase tracking-wider text-muted-foreground font-semibold">Total This Month</div><div className="font-display text-xl font-bold mt-1">{formatNaira(total)}</div></Card>
        {Object.entries(byCat).slice(0,3).map(([k,v])=>(
          <Card key={k} className="p-4"><div className="text-[11px] uppercase tracking-wider text-muted-foreground font-semibold">{k}</div><div className="font-display text-xl font-bold mt-1">{formatNaira(v)}</div></Card>
        ))}
      </div>

      <Card className="p-4">
        <h3 className="font-display font-bold text-sm flex items-center gap-2"><Wallet className="h-4 w-4 text-primary"/> Recent Expenses</h3>
            ...
            <tbody>
              {(exps.data||[]).map(e=>(
                <tr key={e.id} className="border-b border-border last:border-0 hover:bg-muted/40">
                  ...
                  <td className="py-2.5 pr-3"><Badge tone="info">{e.category}</Badge></td>
