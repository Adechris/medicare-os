...
function POSPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (items.length) {
        setCart(items);
        toast.success(`Loaded prescription for ${p.patient}`);
      }
  ...
  const filtered = useMemo(() => {
    ...
    return list.filter((m) =>
      `${m.name} ${m.generic} ${m.brand}`.toLowerCase().includes(q.toLowerCase()),
    );
  }, [meds.data, q]);
  ...
  const dec = (id: string) =>
    setCart((c) =>
      c.flatMap((x) =>
        x.medicine.id === id ? (x.qty > 1 ? [{ ...x, qty: x.qty - 1 }] : []) : [x],
      ),
    );
  const inc = (id: string) =>
    setCart((c) => c.map((x) => (x.medicine.id === id ? { ...x, qty: x.qty + 1 } : x)));
  ...
  const complete = () => {
    ...
    if (payment === "Cash" && Number(tendered || 0) < total)
    ...
    <div className="grid gap-4 lg:grid-cols-[1fr_400px]">
          ...
          {filtered.map((m) => (
            ...
            >
              ...
              <div className="flex items-center justify-between mt-2">
                ...
                <Badge
                  tone={
                    m.stock === 0 ? "danger" : m.stock <= m.reorderLevel ? "warning" : "success"
                  }
                >
      ...
      <Card className="p-4 flex flex-col h-fit lg:sticky lg:top-20">
        ...
        <div className="mt-3 -mx-1 max-h-[42vh] overflow-y-auto">
          {cart.length === 0 ? (
            <div className="text-center text-xs text-muted-foreground py-8">
              Search and click a medicine to add it to the cart.
            </div>
          ) : (
            <ul className="space-y-2 px-1">
              {cart.map((i) => (
                <li
                  key={i.medicine.id}
                  className="p-2.5 rounded-lg bg-surface-grey border border-border"
                >
                  <div className="flex items-start gap-2">
                    <div className="flex-1 min-w-0">
                      <div className="font-semibold text-sm truncate">{i.medicine.name}</div>
                      <div className="text-[11px] text-muted-foreground">
                        {formatNaira(i.medicine.sellingPrice)} × {i.qty}
                      </div>
                    ...
                    <div className="text-right">
                      <div className="font-semibold text-sm">
                        {formatNaira(i.qty * i.medicine.sellingPrice)}
                      </div>
                      <div className="mt-1 inline-flex items-center border border-border rounded-md">
                        <button
                          onClick={() => dec(i.medicine.id)}
                          className="h-6 w-6 inline-flex items-center justify-center hover:bg-accent"
                        >
                          <Minus className="h-3 w-3" />
                        </button>
                        <span className="px-1.5 text-xs tabular-nums w-6 text-center">{i.qty}</span>
                        <button
                          onClick={() => inc(i.medicine.id)}
                          className="h-6 w-6 inline-flex items-center justify-center hover:bg-accent"
                        >
                        ...
                        <button
                          ...
                          className="h-6 w-6 inline-flex items-center justify-center text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 border-l border-border"
                        >
        ...
        <div className="mt-4 pt-3 border-t border-border space-y-1.5 text-sm">
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center gap-1">
              <input
                ...
                className="w-14 h-7 px-1 text-right rounded border border-border bg-background text-xs"
              />
          ...
          <div className="flex justify-between font-display font-bold text-base pt-1 border-t border-border mt-1.5">
            <span>Total</span>
          ...
          <div className="grid grid-cols-4 gap-1.5">
            {(["Cash", "Card", "Transfer", "Credit"] as const).map((m) => (
              <button
                ...
                className={`h-8 rounded-md text-xs font-semibold border ${payment === m ? "bg-primary border-primary text-primary-foreground" : "border-border hover:bg-accent"}`}
              >
                {m}
              </button>
            ))}
          ...
          {payment === "Cash" && (
            <div className="mt-2 flex items-center gap-2 text-sm">
              ...
              <input
                ...
                className="flex-1 h-9 px-2 rounded-lg border border-border bg-background text-sm"
              />
              <div className="text-xs text-muted-foreground whitespace-nowrap">
                Change: <span className="font-semibold text-foreground">{formatNaira(change)}</span>
              </div>
            </div>
          )}
        </div>

        <Button className="mt-3 w-full h-10 text-base" onClick={complete}>
          Complete Sale
        </Button>
      </Card>
      ...
      {receiptOpen && (
        ...
        >
          <div
            className="bg-card rounded-2xl max-w-sm w-full p-6 print-area"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="text-center">
              <div className="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-primary text-primary-foreground font-bold">
                M+
              </div>
            ...
            <div className="mt-4 text-xs space-y-0.5 border-y border-dashed py-3">
              <div className="flex justify-between">
                <span>Receipt</span>
                <span className="font-mono">{receiptOpen.receiptNo}</span>
              </div>
              <div className="flex justify-between">
                <span>Date</span>
            ...
            <div className="mt-3 flex justify-between text-base font-bold">
              <span>Total</span>
              <span>{formatNaira(receiptOpen.total)}</span>
            </div>
            {receiptOpen.change > 0 && (
              <div className="text-xs flex justify-between">
                <span>Change</span>
                <span>{formatNaira(receiptOpen.change)}</span>
              </div>
            )}
            <div className="mt-5 flex gap-2">
              ...
              <Button className="flex-1" onClick={() => setReceiptOpen(null)}>
