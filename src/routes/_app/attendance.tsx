  ...
  return (
    <div>
      <PageHeader title="Attendance" description="Today's clock-ins and shift records"
        actions={<>
          {user && (isClockedIn
            ? <Button variant="danger" onClick={()=>clockOut.mutate()} disabled={clockOut.isPending}><LogOut className="h-4 w-4"/> Clock Out</Button>
            : <Button onClick={()=>clockIn.mutate()} disabled={clockIn.isPending}><LogIn className="h-4 w-4"/> Clock In</Button>)}
          <Button variant="outline" onClick={()=>toast.success("Export started")}>Export CSV</Button>
        </>}/>

      {user && (
        <Card className="p-4 mb-4 flex items-center gap-4">
          <div className={`h-10 w-10 rounded-full flex items-center justify-center ${isClockedIn ? "bg-emerald-100 text-emerald-700" : "bg-muted text-muted-foreground"}`}><Clock className="h-5 w-5"/></div>
          ...
          {me && <Badge tone={me.status==="Present"?"success":me.status==="Late"?"warning":"danger"}>{me.status}</Badge>}
        </Card>
      )}
          ...
          {list.map(a=>(
            <li key={a.id} className="py-2.5 flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-muted-foreground">
                  {a.clockIn ? `In ${a.clockIn}` : "Not clocked in"}{a.clockOut ? ` · Out ${a.clockOut}` : ""}
                </div>
              </div>
