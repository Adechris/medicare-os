...
function MedicinesPage() {
  ...
  const filtered = useMemo(() => {
    return (meds.data || []).filter((m) => {
      if (q && !`${m.name} ${m.generic} ${m.brand}`.toLowerCase().includes(q.toLowerCase()))
        return false;
  ...
  return (
    <div>
      <PageHeader
        title="Medicines"
        description={`${meds.data?.length ?? 0} SKUs in your catalog`}
        actions={
          <>
        ...
        <div className="mt-4 overflow-x-auto">
          {meds.isLoading ? (
            ...
            <EmptyState
              ...
              description="Try adjusting filters or add a new medicine."
              action={
                <Button onClick={() => setAddOpen(true)}>
                  <Plus className="h-4 w-4" /> Add Medicine
                </Button>
            ...
            <table className="w-full text-sm min-w-[1100px]">
              ...
              <tbody>
                {filtered.map((m) => {
                  const margin = (
                    ((m.sellingPrice - m.buyingPrice) / m.sellingPrice) *
                    100
                  ).toFixed(0);
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-2.5 pr-3">
                        ...
                        <div
                          className={`text-[11px] ${days <= 7 ? "text-expiry-critical font-semibold" : days <= 30 ? "text-expiry-warning" : "text-muted-foreground"}`}
                        >
                      ...
                      <td className="py-2.5">
                        <Badge
                          tone={
                            status === "in" ? "success" : status === "low" ? "warning" : "danger"
                          }
                        >
                          {status === "in"
                            ? "In Stock"
