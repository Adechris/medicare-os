...
function PrescriptionsPage() {
  ...
  const dispense = (p: Prescription) => {
    try {
      sessionStorage.setItem("pos_dispense", JSON.stringify(p));
    } catch {}
    navigate({ to: "/pos" });
  };
  ...
  return (
    <div>
      <PageHeader
        title="Prescriptions"
        description="Pending and fulfilled prescriptions"
        actions={
        ...
        {(presc.data || []).map((p) => (
          <Card key={p.id} className="p-4">
            <div className="flex items-start gap-3">
              <div className="h-10 w-10 rounded-lg bg-violet-100 text-violet-600 dark:bg-violet-500/15 flex items-center justify-center">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[11px] text-muted-foreground mt-0.5">
                  Issued {formatDateShort(p.dateIssued)} · Valid until{" "}
                  {formatDateShort(p.validUntil)}
                </div>
              </div>
            </div>
            ...
            <div className="mt-3 flex gap-2">
              <Button variant="outline" className="flex-1" onClick={() => setView(p)}>
                View Details
              </Button>
              <Button className="flex-1" onClick={() => dispense(p)}>
                Dispense from POS
              </Button>
            </div>
          </Card>
      ...
      {view && (
        <Modal
          ...
          title={`Prescription · ${view.patient}`}
          description={`${view.doctor} · ${view.hospital}`}
          size="lg"
        >
          <div className="space-y-3 text-sm">
            <div className="grid grid-cols-2 gap-3 text-xs">
              ...
              <div>
                ...
                {formatDateShort(view.dateIssued)}
              </div>
              <div>
                <span className="text-muted-foreground">Valid until: </span>
                {formatDateShort(view.validUntil)}
              </div>
            </div>
