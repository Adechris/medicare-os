...
function TransactionsPage() {
  ...
  const visible = useMemo(() => {
    ...
    if (q)
      list = list.filter((t) =>
        `${t.receiptNo} ${t.customerName}`.toLowerCase().includes(q.toLowerCase()),
      );
  ...
  return (
    <div>
      <PageHeader
        title="Transactions"
        description={
          user?.role === "cashier" ? "Your sales history" : "All sales across the pharmacy"
        }
        actions={
      ...
      <div className="grid gap-3 grid-cols-2 lg:grid-cols-4 mb-4">
        <Card className="p-4">
          ...
          <div className="font-display text-xl font-bold mt-1">{visible.length}</div>
        </Card>
        <Card className="p-4">
          <div className="text-[11px] uppercase tracking-wider text-muted-foreground font-semibold">
            Total Revenue
          </div>
          <div className="font-display text-xl font-bold mt-1">{formatNaira(todayRev)}</div>
        </Card>
        <Card className="p-4">
          <div className="text-[11px] uppercase tracking-wider text-muted-foreground font-semibold">
            Cash
          ...
          <div className="font-display text-xl font-bold mt-1">
            {formatNaira(
              visible.filter((t) => t.paymentMethod === "Cash").reduce((s, t) => s + t.total, 0),
            )}
          </div>
        </Card>
        <Card className="p-4">
          ...
          <div className="font-display text-xl font-bold mt-1">
            {formatNaira(
              visible
                .filter((t) => t.paymentMethod === "Card" || t.paymentMethod === "Transfer")
        ...
        <div className="flex flex-wrap gap-2">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              ...
              className="w-full pl-9 h-9 rounded-lg border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-primary"
            />
          </div>
          <select
            value={method}
        ...
        >
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => setOpenId(null)}
                className="ml-auto h-8 w-8 inline-flex items-center justify-center rounded-md hover:bg-accent"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
            <div className="mt-3 text-xs text-muted-foreground">
              {formatDate(opened.date)} · {opened.cashier}
            </div>
            ...
            <div className="mt-4 pt-3 border-t border-border space-y-1 text-sm">
              <div className="flex justify-between">
                ...
                <span>{formatNaira(opened.subtotal)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Discount</span>
                <span>−{formatNaira(opened.discount)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">VAT</span>
                <span>{formatNaira(opened.vat)}</span>
              </div>
              <div className="flex justify-between font-display font-bold text-base pt-1.5">
                <span>Total</span>
                <span>{formatNaira(opened.total)}</span>
              </div>
            </div>
