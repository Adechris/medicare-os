  ...
  const inner = (
    <Card className="p-4 hover:shadow-card-lg transition-shadow">
      <div className="flex items-center gap-3">
        <div
          className={`h-10 w-10 rounded-lg bg-gradient-to-br ${tones[tone]} flex items-center justify-center`}
        >
    ...
    <div>
      ...
      <div className="grid gap-3 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <KPI
          ...
          tone="violet"
          label="Transactions"
          value={formatNumber(txnsToday)}
          sub="across all cashiers"
        />
        <KPI
          icon={Pill}
          tone="primary"
        ...
        <KPI
          ...
          value={formatNaira(monthExpenses)}
          sub="May 2026"
        />
      </div>
            ...
            <div className="inline-flex rounded-lg border border-border p-0.5 text-xs">
              {(["daily", "weekly", "monthly"] as const).map((g) => (
                <button
                  ...
                  className={`px-2.5 py-1 rounded-md capitalize ${granularity === g ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-accent"}`}
                >
                  {g}
                </button>
              ))}
      ...
      <div className="grid gap-4 mt-4 grid-cols-1 lg:grid-cols-3">
        <Card className="p-4">
          <h3 className="font-display font-bold text-sm flex items-center gap-2">
            <AlertOctagon className="h-4 w-4 text-expiry-critical" /> Expiry Alerts
          </h3>
          <ul className="mt-3 space-y-2">
            <li className="flex items-center justify-between p-2 rounded-md bg-red-50 dark:bg-red-500/10">
              <span className="text-sm">Expired (today)</span>
              <Badge tone="danger">1</Badge>
            </li>
            <li className="flex items-center justify-between p-2 rounded-md bg-orange-50 dark:bg-orange-500/10">
              <span className="text-sm">Within 7 days</span>
              <Badge tone="warning">2</Badge>
            </li>
            <li className="flex items-center justify-between p-2 rounded-md bg-amber-50 dark:bg-amber-500/10">
              <span className="text-sm">Within 30 days</span>
              <Badge tone="amber">4</Badge>
            </li>
          </ul>
          <Link
            to="/expiry"
          ...
          >
            View all expiry alerts <ArrowRight className="h-3 w-3" />
          </Link>
        </Card>

        <Card className="p-4">
          <h3 className="font-display font-bold text-sm flex items-center gap-2">
            <PackagePlus className="h-4 w-4 text-amber-600" /> Low Stock
          </h3>
          <ul className="mt-3 space-y-2">
            {(meds.data || [])
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Medicines needing your attention today."
        actions={
          <>
            <Button variant="outline" onClick={() => setAddMed(true)}>
              <Plus className="h-4 w-4" /> Add Medicine
            </Button>
            <Button variant="outline" onClick={() => navigate({ to: "/inventory" })}>
              <Pill className="h-4 w-4" /> Check Inventory
            </Button>
            <Button onClick={() => navigate({ to: "/pos" })}>
              <Plus className="h-4 w-4" /> New Sale
            </Button>
      ...
      <div className="grid gap-3 grid-cols-2 lg:grid-cols-4">
        <KPI
          ...
          label="Low Stock"
          value={formatNumber(
            meds.data?.filter((m) => m.stock <= m.reorderLevel && m.stock > 0).length ?? 0,
          )}
          to="/inventory"
        />
        <KPI
          ...
          tone="danger"
          label="Expiring Soon"
          value={formatNumber(meds.data?.filter((m) => daysUntil(m.expiryDate) <= 30).length ?? 0)}
          to="/expiry"
        />
        <KPI
          icon={FileText}
          ...
          value={formatNumber(presc.data?.length ?? 0)}
          to="/prescriptions"
        />
        ...
        <ul className="mt-3 divide-y divide-border">
          {(presc.data || []).map((p) => (
            <li key={p.id} className="py-2.5 flex items-center gap-3">
              ...
              <Badge
                tone={
                  p.status === "Filled" ? "success" : p.status === "Pending" ? "warning" : "info"
                }
              >
                {p.status}
              </Badge>
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Your sales summary for today."
        actions={
          <Link to="/pos">
            <Button>
      ...
      <div className="grid gap-3 grid-cols-2 lg:grid-cols-3">
        <KPI
          ...
          value={formatNaira(my.reduce((s, t) => s + t.total, 0))}
        />
        <KPI
          icon={ShoppingCart}
          tone="violet"
          label="My Transactions"
          value={formatNumber(my.length)}
        />
        <KPI
          icon={Users}
          tone="success"
          label="Customers Served"
          value={formatNumber(new Set(my.map((t) => t.customerName)).size)}
        />
      </div>
      <Card className="p-4 mt-4">
          ...
          {my.slice(0, 8).map((t) => (
            <li key={t.id} className="py-2.5 flex items-center gap-3">
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-muted-foreground">
                  {t.customerName} · {formatDateShort(t.date)}
                </div>
              </div>
              <Badge tone="info">{t.paymentMethod}</Badge>
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Stock health, expiries and supplier activity."
        actions={
          <>
      ...
      <div className="grid gap-3 grid-cols-2 lg:grid-cols-4">
        <KPI
          ...
          tone="primary"
          label="Total Stock"
          value={formatNumber(meds.data?.reduce((s, m) => s + m.stock, 0) ?? 0)}
        />
        <KPI
          icon={AlertTriangle}
          tone="warning"
        ...
        <KPI
          ...
          tone="danger"
          label="Out of Stock"
          value={formatNumber(meds.data?.filter((m) => m.stock === 0).length ?? 0)}
        />
        <KPI
          icon={AlertOctagon}
          tone="danger"
            ...
            {(meds.data || [])
              ...
              .map((m) => (
                <li key={m.id} className="py-2.5 flex items-center gap-3">
                  ...
                  <Badge tone={m.stock === 0 ? "danger" : "warning"}>
                    {m.stock === 0 ? "Out" : "Low"}
                  </Badge>
          ...
          <ul className="mt-3 divide-y divide-border">
            {(sup.data || []).map((s) => (
              <li key={s.id} className="py-2.5">
                ...
                <div className="text-xs text-muted-foreground">
                  {s.name} · Last order {formatDateShort(s.lastOrder)}
                </div>
              </li>
            ))}
          </ul>
