...
function InventoryPage() {
  ...
  const quick = useMutation({
    ...
    onSuccess: (_d, v) => {
      toast.success(`Stock ${v.delta > 0 ? "+" : ""}${v.delta}`);
  ...
  return (
    <div>
      <PageHeader
        title="Inventory"
        description="Stock levels, movements and purchase orders"
        actions={
          <>
            ...
            <Button
              onClick={() =>
                data[0] && setAdjust({ id: data[0].id, name: data[0].name, stock: data[0].stock })
              }
            >
              Adjust Stock
            </Button>
      ...
      <div className="grid gap-3 grid-cols-2 lg:grid-cols-4">
        <Card className="p-4">
          ...
          <div className="font-display text-xl font-bold mt-1">{formatNumber(data.length)}</div>
        </Card>
        <Card className="p-4">
          <div className="text-[11px] uppercase tracking-wider text-muted-foreground font-semibold">
            Stock Value
          </div>
          <div className="font-display text-xl font-bold mt-1">{formatNaira(totalValue)}</div>
        </Card>
        <Card className="p-4">
          ...
          <div className="font-display text-xl font-bold mt-1">{formatNumber(low)}</div>
        </Card>
        <Card className="p-4">
          <div className="text-[11px] uppercase tracking-wider text-muted-foreground font-semibold flex items-center gap-1">
            <AlertOctagon className="h-3 w-3 text-red-500" /> Out of Stock
          </div>
          <div className="font-display text-xl font-bold mt-1">{formatNumber(out)}</div>
        </Card>
      </div>
                  ...
                  <tr key={m.id} className="border-b border-border last:border-0 hover:bg-muted/40">
                    <td className="py-2.5 pr-3">
                      <div
                        ...
                        onClick={() => setAdjust({ id: m.id, name: m.name, stock: m.stock })}
                      >
                        {m.name}
                      </div>
                    ...
                    <td className="py-2.5 pr-3 font-semibold">
                      {formatNaira(m.stock * m.buyingPrice)}
                    </td>
                    <td className="py-2.5 pr-3">
                      <Badge
                        tone={status === "in" ? "success" : status === "low" ? "warning" : "danger"}
                      >
                    ...
                    <td className="py-2.5 text-right">
                      <div className="inline-flex border border-border rounded-md">
                        <button
                          title="Add 1"
                          onClick={() => quick.mutate({ id: m.id, delta: +1 })}
                          disabled={quick.isPending}
                          className="h-7 w-7 inline-flex items-center justify-center hover:bg-accent text-emerald-600 disabled:opacity-50"
                        >
                        ...
                        <button
                          title="Remove 1"
                          onClick={() => quick.mutate({ id: m.id, delta: -1 })}
                          disabled={quick.isPending || m.stock === 0}
                          className="h-7 w-7 inline-flex items-center justify-center hover:bg-accent text-red-600 border-l border-border disabled:opacity-50"
                        >
                        ...
                        <button
                          title="Custom adjust"
                          onClick={() => setAdjust({ id: m.id, name: m.name, stock: m.stock })}
                          className="h-7 px-2 text-xs hover:bg-accent border-l border-border"
                        >
                          …
                        </button>
                      </div>
      ...
      {adjust && (
        <AdjustStockDialog
          ...
          medicineId={adjust.id}
          medicineName={adjust.name}
          currentStock={adjust.stock}
          initialDelta={adjust.delta}
        />
      )}
