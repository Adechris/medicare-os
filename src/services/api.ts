...
export const api = {
  ...
  createMedicine: (m: Omit<Medicine, "id">) => {
    const x: Medicine = { ...m, id: id("m") };
    MEDICINES.unshift(x);
    return delay(x);
  },
  ...
  createCustomer: (
    c: Omit<
      Customer,
      "id" | "totalPurchases" | "outstandingDebt" | "lastVisit" | "loyaltyPoints" | "status"
    >,
  ) => {
    const x: Customer = {
      ...c,
  ...
  createExpense: (e: Omit<Expense, "id">) => {
    const x: Expense = { ...e, id: id("e") };
    EXPENSES.unshift(x);
    return delay(x);
  },
